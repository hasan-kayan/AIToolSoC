...
for i in range(len(class_data_list)):
    ...
    train_test = {}
    X_train, X_test, y_train, y_test = train_test_split(class_data_list[i].data['x'].values.reshape(-1,1), class_data_list[i].data['y'].values, test_size=0.2, random_state=42)
    svr.fit(X_train,y_train)
    y_pred = svr.predict(X_test)
    class_data_list[i].intercept = svr.intercept_
...
for i in range(len(class_data_list)):
    ...
    print(class_data_list[i].r2_score*100)
